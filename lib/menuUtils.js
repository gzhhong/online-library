...
export async function getMenuSettingsByRole(roleId) {
  try {
    ...
    return menuSettings
      ...
      .filter(item => {
        // 如果roleId为0，则不必过滤，全部返回
        if (roleId === 0) return true;
        
        // 如果item.roleIds为空，则只有当roleId为0时才返回
        // 即item.roleIds为空的item只能被roleId=0的用户看到
        if (item.roleIds.length === 0) return false;
...
export function convertToReactMenuItems(menuTree, iconMapper = null) {
  // 如果只有一个根节点且其level为0，直接返回其子节点
  if (menuTree.length === 1 && menuTree[0].level === 0 && menuTree[0].children && menuTree[0].children.length > 0) {
    return convertToReactMenuItems(menuTree[0].children, iconMapper);
  }
