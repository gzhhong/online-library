...
console.log('COS Module Loaded. Environment:', {
  ...
  RUNENV: process.env.CLOUD_ENV_ID,
  ...
  try {
    cosInstance = new CosSDK({
      getAuthorization: async function (options, callback) {
        try {
          console.log('Getting COS authorization...', options);
...
export async function uploadToCOS(localPath, cloudPath) {
  ...
  try {
    ...
    if (fileSize > 1024 * 1024) {
      console.log('Using slice upload for large file:', {
        fileSize,
        path: localPath,
        cloudPath
      });
      return new Promise((resolve, reject) => {
        cos.sliceUploadFile({
          ...
          Key: cloudPath,
          FilePath: localPath,
          SliceSize: 1024 * 100, // 每片100KB
          AsyncLimit: 3, // 分片上传的并发数，默认值3
          Headers: {
            'x-cos-meta-fileid': metadata.respdata.x_cos_meta_field_strs[0]
          },
          onProgress: function(progressData) {
            console.log('Slice upload progress:', {
          ...
          if (err) {
            console.error('Slice upload error:', {
              error: err,
              errorCode: err.code,
              errorMessage: err.message,
              requestId: err.headers?.['x-cos-request-id'],
            ...
            console.log('Slice upload success:', {
              requestId: data.headers?.['x-cos-request-id'],
